...
import { aliasedTable, between, ConsoleLogWriter, sql } from "drizzle-orm";
...
app.post("/user", bearerAuth({ token }), async (c) => {
  ...
  const user = await db
    .insert(users)
    .values({ employeeid: employeeid, email: email, password: hashedPassword });
  return c.json(user);
});
...
app.post("/milage", bearerAuth({ token }), async (c) => {
  try {
    ...
    console.log(date);
    const newmilage = await db
      .insert(milage)
      .values({
        equipmentid: equipmentid,
        milagemeter: milagemeter,
...
app.post("/cancel-tasksch", async (c) => {
  const { equiptaskschid } = await c.req.json();
  console.log(`task shedule number-${equiptaskschid}`);
  try {
    await db
      .update(equipmenttasksch)
      .set({ cancelled: true })
      .where(eq(equipmenttasksch.equipmenttaskschid, equiptaskschid));
    return c.json({ message: equiptaskschid });
  } catch (error:any) {
    console.error(error);
    return c.json({sucess:false,message:error.message,code:500});
  }
});
app.post("/cancel-maintainence", async (c) => {
  const { maintenance } = await c.req.json();
  console.log(`Matintenance number-${maintenance}`);
  try {
    await db
      .update(reactiveMaintenance)
      .set({ cancelled: true })
      .where(eq(reactiveMaintenance.maintenanceid, maintenance));
    return c.json({ message: maintenance });
  } catch (error:any) {
    console.error(error);
    return c.json({sucess:false,message:error.message,code:500});
  }
});
app.post("/cancel-workorder", async (c) => {
  const { workorderid } = await c.req.json();
  ...
  try {
    ...
    return c.json({ message: `cancelled workorder: ${workorderid} `});
  } catch (error:any) {
    console.error(error);
    return c.json({sucess:false,message:error.message,code:500});
  }
});
app.post("/cancel-fuelentry", async (c) => {
  const { fuelentryid } = await c.req.json();
  
  try {
    await db.update(fuelentry).set({cancelled:true}).where(eq(fuelentry.fuelentryid,fuelentryid))
    return c.json({ message: fuelentryid },200);
  } catch (error:any) {
    console.error(error);
    return c.json({sucess:false,message:error.message,code:500});
  }
});
app.post("/cancel-meter", async (c) => {
  const { meterupdateid } = await c.req.json();
  
  try {
    await db.update(milage).set({cancelled:true}).where(eq(milage.meterupdateid,meterupdateid))
    return c.json({ message: meterupdateid },200);
  } catch (error:any) {
    console.error(error);
    return c.json({sucess:false,message:error.message,code:500});
  }
});
app.post("/cancel-task", async (c) => {
  const { taskid } = await c.req.json();
  console.log(`task number-${taskid}`);
  try {
    await db
      .update(tasks)
      .set({ cancelled: true })
      .where(eq(tasks.taskid, taskid));
    return c.json({ message: taskid });
  } catch (error:any) {
    console.error(error);
    return c.json({sucess:false,message:error.message,code:500});
  }
  ...
  try {
    const updatedTask = await db
      ...
      return c.json({
        ...
        updatedTask: updatedTask[0] // Send the updated task data in the response
      });
  } catch (error) {
    console.error(error);
    return c.json({ error: "Failed to update Equipment" }, 500);
...
app.post("/tasks", async (c) => {
  try {
    const { taskname, frequency, frequencyunit, advnotice } =
      await c.req.json();
...
app.post("/tasksch", async (c) => {
  try {
    ...
    const nextparformh =
      lastperformeh > 0
        ? lastperformeh + frequencytime
        : frequencytimeunit === "Hours"
        ? frequencytime
        : 0;
    const nextperformkm =
      lastperformedkm > 0 && frequencykm > 0
        ? lastperformedkm + frequencykm
        : frequencykm;
        const lastperformdate = lastperformed
    ...
    const nextperformd =
      frequencytime > 0
        ? frequencytimeunit === "Months"
          ? new Date(
              new Date(lastperformdate).setMonth(
                lastperformdate.getMonth() + frequencytime
              )
            )
          : frequencytimeunit === "Days"
          ? new Date(
              new Date(lastperformdate).setDate(
                lastperformdate.getDate() + frequencytime
              )
            )
          : null
        : null;

    const result = await db.insert(equipmenttasksch).values({
      equipmentid: equipmentid,
      taskid: taskid,
      frequencytime: frequencytime,
      frequencytimeunit: frequencytimeunit,
      advancenoticetime: advancenoticetime,
      lastperformed: !lastperformed ? new Date("01/01/1900") : lastperformed,
      lastperformeh: lastperformeh,
      notes: notes,
      status: status,
      frequencykm: frequencykm,
      lastperformedkm,
      advanceNoticekm: advanceNoticekm,
      nextschh: nextparformh,
      nextschk: nextperformkm,
      nextschdate:nextperformd?.toISOString()
    });
    return c.json({ success: true, data: result }, 200);
  } catch (error) {
...
app.post("/reactivemaintenance", async (c) => {
  try {
    ...
    console.error(error);
    return c.json({ success:false,message:error.message,code:500 }, 500);
  }
});
...
app.get("/equipments", async (c) => {
  ...
  return c.json(myequipment);
});

  app.get("/equipmentlist", async (c) => {
    try {
      // Fetch the columns separately
      const myequipment = await db
        .select({
          id: equipment.equipmentid,
      ...
      // Concatenate the description in JavaScript before returning
      const formattedEquipment = myequipment.map(e => ({
        id: e.id.toString(),
        desc: `${e.equipmentname}/${e.model}`,
      }));
  
      return c.json(formattedEquipment);
    } catch (error) {
      return c.json({ error: "Error fetching equipment list" },500);
    }
  });
...
app.get("/tasklist", async (c) => {
  const tasklist = await db
    .select({
      ...
      frequencyunit:tasks.frequencyunit,
      advancenotice:tasks.advancenotice
    })
    .from(tasks)
    .where(eq(tasks.cancelled,false));
...
app.get("/fuelhistory/:equipmentid", async (c) => {
  ...
  const fuelhistory = await db
    .select({
      fuelentryid:fuelentry.fuelentryid,
    ...
    .from(fuelentry)
    .where(and(eq(fuelentry.equipmentid, equipid),eq(fuelentry.cancelled,false)));
...
app.get("/workorderhistory/:equipmentid", async (c) => {
  ...
  const workorderhistory = await db
    ...
    .leftJoin(
      equipmenttasksch,
      eq(workorder.taskid, equipmenttasksch.equipmenttaskschid)
    )
...
app.get("/reactivehistory/:equipmentid", async (c) => {
  const equipid = parseInt(c.req.param("equipmentid"));
  const employee1 = aliasedTable(employees, "employee1")
  const reactivehistory = await db
    .select({
      maintenanceid:reactiveMaintenance.maintenanceid,
      ...
      reportedby: employee1.employeename,
      asssigtechnician: employees.employeename,
      ...
      equipment:reactiveMaintenance.equipmentid,
    })
    .from(reactiveMaintenance)
    .leftJoin(employees,eq(reactiveMaintenance.assignedtechnician,employees.employeeid))
    .leftJoin(employee1,eq(reactiveMaintenance.reportedby,employee1.employeeid))
    .where(and(eq(reactiveMaintenance.equipmentid, equipid),eq(reactiveMaintenance.cancelled,false)));
...
app.get("/expensehistory/:equipmentid", async (c) => {
  ...
  const expensehistory = await db
    ...
    .from(expenses)
    .leftJoin(equipment, eq(expenses.equipmentid, equipment.equipmentid))
...
app.get("/milagehistory/:equipmentid", async (c) => {
  ...
  const milagehistory = await db
    .select({
      meterupdateid:milage.meterupdateid,
      equipmentname: equipment.equipmentname,
      model: equipment.model,
    ...
    .from(milage)
    .leftJoin(equipment, eq(milage.equipmentid, equipment.equipmentid))
    .where(and(eq(milage.equipmentid, equipid),eq(milage.cancelled,false)));

  return c.json(milagehistory);
});
app.get("/workorderlist", async (c) => {
  const wolist = await db
    ...
    .leftJoin(employees, eq(workorder.assignee, employees.employeeid))
    .leftJoin(vendors, eq(workorder.vendorid, vendors.vendorid))
    .where(and(ne(workorder.status, "completed"),eq(workorder.cancelled,false)));
...
FROM
  ...
  where et.equipmentid=${equipid} and et.cancelled=false`);
  return c.json(tasklist);
});
...
app.get("/notificationcount", async (c) => {
  ...
  `);
    ...
    const finalMessage =
      messageParts.length && messageParts.some((part) => part)
        ? `There are ${messageParts.filter((part) => part).join(" and ")}`
        : "No Due Maintenence";
...
app.get("/sparedetails", async (c) => {
  ...
  const result =
    await db.execute(sql`SELECT s.partid as part_code,s.partname AS spare_part_name,
  ...
  const formattedParts: parts[] = parsedRows.reduce(
    (acc: parts[], part: SparePart) => {
      // Check if the partname already exists in acc
      const existingPart = acc.find(
        (item) => item.part_code === part.part_code
      );

      if (existingPart) {
        // Add equipment description to existing part
        existingPart.equipment_id.push(part.equipment_id);
        existingPart.equipmentdesc.push(
          `${part.equipment_name}/${part.equipment_model}`
        );
      } else {
...
app.get("/workorderdet/:equipid/:fromdate/:todate", async (c) => {
  const eid = parseInt(c.req.param("equipid"));
const fromdate=c.req.param("fromdate")
const todate=c.req.param("todate")
  // Query using Drizzle ORM
  const result = await db
    .select({
      id: workorder.workorderid,
    ...
    .leftJoin(
      employees,
      eq(workorder.assignee, employees.employeeid)
    ).leftJoin(
      equipmenttasksch,
      eq(workorder.taskid, equipmenttasksch.equipmenttaskschid)
    ).leftJoin(tasks, eq(equipmenttasksch.taskid, tasks.taskid))
    .where(and(eq(workorder.equipmentid, eid),between(workorder.workcompletiontime,fromdate,todate)));
...
app.get("/maintenancedet/:maintenanceid", async (c) => {
  ...
  const result = await db
    ...
    .leftJoin(
      employees,
      eq(reactiveMaintenance.assignedtechnician, employees.employeeid)
    )
    .where(eq(reactiveMaintenance.maintenanceid, mid));

  return c.json(result);
});
app.get("/maintenancedet/:equipid/:fromdate/:todate", async (c) => {
  const eid = parseInt(c.req.param("equipid"));
const fromdate=c.req.param("fromdate")
const todate=c.req.param("todate")
  // Query using Drizzle ORM
  const result = await db
    ...
    .where(and(eq(reactiveMaintenance.equipmentid, eid),between(reactiveMaintenance.completiondate,fromdate,todate)));
...
app.get("/equipcost/:equipid/month/:month", async (c) => {
  ...
  const result = await db
    ...
    .where(
      and(
        eq(expenses.equipmentid, equipid),
        gt(expenses.trandate, targetDateString)
      )
    );
  ...
  try {
    ...
    for (let i = 5; i >= 0; i--) {
      const month = dayjs()
        ...
        .startOf("month")
        .format("YYYY-MM");
      const result = results.rows.find(
        (r: any) => dayjs(r.month).format("YYYY-MM") === month
      );
      months.push(month);
